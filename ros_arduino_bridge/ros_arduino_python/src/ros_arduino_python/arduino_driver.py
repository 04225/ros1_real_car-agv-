...
class Arduino:
    ...
    def execute_array(self, cmd):
        ...
        try:
            ...
            while attempts < ntries and (values == '' or values == 'Invalid Command' or values == [] or values is None):
                try:
                    self.port.flushInput()
                    self.port.write((cmd + '\r').encode())
                    values = self.recv_array()
    ...
    def execute_ack(self, cmd):
        ...
        try:
            self.port.write((cmd + '\r').encode())
            ack = self.recv(self.timeout)
            while attempts < ntries and (ack == '' or ack == 'Invalid Command' or ack is None):
                try:
                    self.port.flushInput()
                    self.port.write((cmd + '\r').encode())
                    ack = self.recv(self.timeout)
